...
class BitAPI:
    ...
    def ticker(
        ...
        r = requests.get(
            f"{self.url}/pubticker/" + symbol, verify=True
        )  # <== UPDATED TO LATEST VERSION OF BFX!
        rep = r.json()
        try:
            rep["last_price"]
        except KeyError:
    ...
    def stats(self, symbol="btcusd"):  # Various statistics about the requested pairs.
        r = requests.get(
            f"{self.url}/stats/" + symbol, verify=True
        )  # <== UPDATED TO LATEST VERSION OF BFX!
        return r.json()
    ...
    def active_orders(self):  # view your active orders.
        payload = {"request": "/v1/orders", "nonce": self.__gen_nonce()}
    ...
    def place_order(
        self, amount, price, side, ord_type, symbol="btcusd", exchange="bitfinex"
    ):  # submit a new order.
        payload = {
    ...
    def delete_order(self, order_id):  # cancel an order.
        ...
        signed_payload = self.__payload_packer(payload)
        r = requests.post(
            f"{self.url}/order/cancel", headers=signed_payload, verify=True
        )
        rep = r.json()
    ...
    def delete_all_order(self):  # cancel an order.
        ...
        signed_payload = self.__payload_packer(payload)
        r = requests.post(
            f"{self.url}/order/cancel/all", headers=signed_payload, verify=True
        )
        return r.json()

    def balances(self):  # see your balances.
        payload = {"request": "/v1/balances", "nonce": self.__gen_nonce()}
